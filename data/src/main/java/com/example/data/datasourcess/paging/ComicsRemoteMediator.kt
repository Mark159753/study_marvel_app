...
): RemoteMediator<Int, ComicResponse>() {

    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, ComicResponse>
    ): MediatorResult {
...
//                    throw InvalidObjectException("Remote key should not be null for $loadType")
        ...
        val apiResponse = apiService.getComics(page, state.config.pageSize)
            ...
            is LoadStatus.Success ->{
                ...
                marvelLocalDatabase.withTransaction {
                    if (loadType == LoadType.REFRESH){
                        marvelLocalDatabase.getRemoteKeysDao().deleteByItemType(RemoteKeys.COMICS_TYPE)
                        marvelLocalDatabase.getComicsDao().deleteAll()
                    }
                    ...
                    val keys = items.map{
                        RemoteKeys(RemoteKeys.COMICS_TYPE, it.id, prevKey, nextKey)
                    }
                    marvelLocalDatabase.getRemoteKeysDao().insert(keys)
                    marvelLocalDatabase.getComicsDao().insert(items)
                }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ComicResponse>): RemoteKeys? {
        return state.pages.lastOrNull() { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { item ->
                marvelLocalDatabase.getRemoteKeysDao().remoteKeysRepoId(item.id, RemoteKeys.COMICS_TYPE)
            }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, ComicResponse>): RemoteKeys? {
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { characterId ->
                marvelLocalDatabase.getRemoteKeysDao().remoteKeysRepoId(characterId, RemoteKeys.COMICS_TYPE)
            }
        }
    }
}